...
export function Countdown() {
  ...
  if (parts.expired) {
    return (
      <p className="font-display text-2xl text-temple sm:text-3xl">
        Today is the day
      </p>
    );
  }
      ...
      {values.map((value, i) => (
        <div key={labels[i]} className="flex items-center gap-3 sm:gap-5">
          <div className="flex min-w-[4.5rem] flex-col items-center sm:min-w-[5.5rem]">
            <span className="font-display text-3xl font-medium tabular-nums text-temple-deep sm:text-4xl md:text-5xl">
