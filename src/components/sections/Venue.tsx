    ...
    <section id="venue" className="relative px-6 py-20 sm:py-28">
      ...
      <FadeIn>
        <SectionHeading
          ...
          title="The Venues"
          subtitle="Join us at settings chosen for blessing, beauty, and celebration."
        />
      </FadeIn>
        ...
        {wedding.events.map((event) => (
          <StaggerItem key={event.id}>
            <GlassCard className="flex h-full flex-col px-8 py-11 text-center sm:px-10">
              ...
              <p className="mt-5 font-body text-[10px] uppercase tracking-[0.28em] text-burgundy">
                {event.title}
              </p>
              <h3 className="mt-2 font-display text-2xl text-burgundy-deep sm:text-3xl">
                {event.venue}
              </h3>
              ...
              <div className="mt-6 space-y-2.5 font-body text-sm text-ink-muted">
                <p className="inline-flex items-center justify-center gap-2">
                  <Calendar className="h-3.5 w-3.5 text-burgundy" strokeWidth={1.5} />
                  {event.date}
                </p>
                <p className="flex items-center justify-center gap-2">
                  <Clock className="h-3.5 w-3.5 text-burgundy" strokeWidth={1.5} />
                  {event.time}
                </p>
              </div>
